			...
			<thead>
				...
				<g:each in="${titulacaoInstanceList}" status="i" var="titulacaoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${titulacaoInstance.id}">${fieldValue(bean: titulacaoInstance, field: "descricao")}</g:link></td>
					
						<td>${fieldValue(bean: titulacaoInstance, field: "grau")}</td>
					
						<td>${fieldValue(bean: titulacaoInstance, field: "cargaHoraria")}</td>
