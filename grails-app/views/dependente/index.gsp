			...
			<thead>
				...
				<g:each in="${dependenteInstanceList}" status="i" var="dependenteInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${dependenteInstance.id}">${fieldValue(bean: dependenteInstance, field: "cpf")}</g:link></td>
					
						<td><g:formatDate date="${dependenteInstance.dataNascimento}" /></td>
					
						<td>${fieldValue(bean: dependenteInstance, field: "funcionario")}</td>
					
						<td>${fieldValue(bean: dependenteInstance, field: "identidade")}</td>
